...
class TbActiveForm extends CActiveForm
{
	 ...
	 * @since 0.10.0
	 */
	public function dateTimepickerRow($model, $attribute, $options = array(), $dateFormat = null, $timeFormat = false)
	{
		...
		if ($model->isNewRecord) {
			if (isset($options['date']['currentDate']))
			...
			if (!empty($model->$attribute)) {
				$date = Yii::app()->dateFormatter->formatDateTime(strtotime($model->$attribute), 'medium', null);
			} else
				$date = '';
		}
