...
(function () {
  ...
  function n(a, h) {
    ...
    0 > b.indexOf("px") && (b = b in c && "none" != a.getComputedStyle("border-style") ? c[b] : 0);
  ...
  function w(a) {
    ...
    f = a.$.rows;
    for (var p = 0, e, g, d, j = 0, o = f.length; j < o; j++)d = f[j], e = d.cells.length, e > p && (p = e, g = d);
    f = g;
    p = new CKEDITOR.dom.element(a.$.tBodies[0]);
    e = p.getDocumentPosition();
    g = 0;
    for (d = f.cells.length; g < d; g++) {
      var j = new CKEDITOR.dom.element(f.cells[g]), o = f.cells[g + 1] && new CKEDITOR.dom.element(f.cells[g + 1]), b = b + (j.$.colSpan || 1), k, i, l = j.getDocumentPosition().x;
      c ? i = l + n(j, "left") : k = l + j.$.offsetWidth - n(j, "right");
      o ? (l = o.getDocumentPosition().x, c ? k = l + o.$.offsetWidth - n(o, "right") : i = l + n(o, "left")) : (l = a.getDocumentPosition().x, c ? k = l : i = l + a.$.offsetWidth);
      j = Math.max(i - k, 3);
  ...
  function A(a) {
    ...
    function b() {
      for (var v = e.rtl, b = v ? l.length : x.length, a = 0; a < b; a++) {
        ...
        CKEDITOR.tools.setTimeout(function (a, b, d, c, e, g) {
          ...
          e && f.setStyle("width", i(e + g * (v ? -1 : 1)))
        }, 0, this, [d, d && t(d), c, c && t(c),
          (!d || !c) && t(f) + n(f, "left") + n(f, "right"), k])
      }
    }
    ...
    function c(a) {
      ...
      for (var a = e.index, b = CKEDITOR.tools.buildTableMap(e.table), c = [], h = [], i = Number.MAX_VALUE, n = i, s = e.rtl, r = 0, w = b.length; r < w; r++) {
        ...
        if (!q || !m || !q.equals(m))q && (i = Math.min(i, t(q))), m && (n = Math.min(n, t(m))), c.push(q), h.push(m)
      }
      x = c;
      l = h;
      y = e.x - i;
    ...
    var r = this.move = function (a) {
      if (!e)return 0;
      if (!j && (a < e.x || a > e.x + e.width))return e = null, j = k = 0, g.removeListener("mouseup", f), d.removeListener("mousedown", c), d.removeListener("mousemove", p), g.getBody().setStyle("cursor", "auto"), s ? d.remove() : d.hide(), 0;
  ...
  var i = CKEDITOR.tools.cssLength, s = CKEDITOR.env.ie && (CKEDITOR.env.ie7Compat || CKEDITOR.env.quirks || 7 > CKEDITOR.env.version);
  CKEDITOR.plugins.add("tableresize", {init:function (a) {
    a.on("contentDom", function () {
      var h;
      a.document.getBody().on("mousemove", function (b) {
        b = b.data;
        if (h && h.move(b.$.clientX))u(b); else {
          var c = b.getTarget(), f;
          if (c.is("table") || c.getAscendant("tbody", 1)) {
            f = c.getAscendant("table", 1);
            if (!(c = f.getCustomData("_cke_table_pillars")))f.setCustomData("_cke_table_pillars", c = w(f)), f.on("mouseout", r), f.on("mousedown", r);
            a:{
              b = b.$.clientX;
              f = 0;
