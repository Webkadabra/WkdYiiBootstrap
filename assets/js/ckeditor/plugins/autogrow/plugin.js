...
(function () {
  var f = function (a) {
    if (a.window) {
      ...
      var b = c.getBody(), d = c.getDocumentElement(), g = a.window.getViewPaneSize().height, c = "BackCompat" == c.$.compatMode ? b : d, b = c.getStyle("overflow-y"), e = c.getDocument(), d = CKEDITOR.dom.element.createFromHtml('<span style="margin:0;padding:0;border:0;clear:both;width:1px;height:1px;display:block;">' + (CKEDITOR.env.webkit ? "&nbsp;" : "") + "</span>", e);
      e[CKEDITOR.env.ie ? "getBody" : "getDocumentElement"]().append(d);
      e = d.getDocumentPosition(e).y + d.$.offsetHeight;
      d.remove();
      c.setStyle("overflow-y", b);
      b = e + (a.config.autoGrow_bottomSpace || 0);
      d = a.config.autoGrow_maxHeight || Infinity;
      b = Math.max(b, void 0 != a.config.autoGrow_minHeight ? a.config.autoGrow_minHeight : 200);
      b = Math.min(b, d);
      b != g && (b = a.fire("autoGrow", {currentHeight:g, newHeight:b}).newHeight, a.resize(a.container.getStyle("width"), b, !0));
  ...
  CKEDITOR.plugins.add("autogrow",
    {init:function (a) {
      if (a.elementMode != CKEDITOR.ELEMENT_MODE_INLINE)a.on("instanceReady", function () {
        if (a.editable().isInline())a.ui.space("contents").setStyle("height", "auto"); else {
          ...
          for (b in c)a.on(b, function (b) {
            var c = a.getCommand("maximize");
            "wysiwyg" == b.editor.mode && (!c || c.state != CKEDITOR.TRISTATE_ON) && setTimeout(function () {
                f(b.editor);
                f(b.editor)
              },
              100)
