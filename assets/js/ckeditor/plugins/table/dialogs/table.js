...
(function () {
  function r(a) {
    for (var f = 0, l = 0, k = 0, d, e = a.$.rows.length; k < e; k++) {
      d = a.$.rows[k];
      for (var c = f = 0, b, g = d.cells.length; c < g; c++)b = d.cells[c], f += b.colSpan;
      f > l && (l = f)
    }
    return l
  }
    ...
    return{title:a.lang.table.title, minWidth:310, minHeight:CKEDITOR.env.ie ? 310 : 280,
      onLoad:function () {
        ...
        if (a)a.on("change", function () {
          ...
          (b = d.getContentElement("info", "txtHeight")) && b.setValue(a, !0)
        })
      }, onShow:function () {
        var d = a.getSelection(), e = d.getRanges(), c, b = this.getContentElement("info", "txtRows"), g = this.getContentElement("info", "txtCols"), p = this.getContentElement("info", "txtWidth"), h = this.getContentElement("info",
          "txtHeight");
        "tableProperties" == f && ((d = d.getSelectedElement()) && d.is("table") ? c = d : 0 < e.length && (CKEDITOR.env.webkit && e[0].shrink(CKEDITOR.NODE_ELEMENT), c = a.elementPath(e[0].getCommonAncestor(!0)).contains("table", 1)), this._.selectedElement = c);
        c ? (this.setupContent(c), b && b.disable(), g && g.disable()) : (b && b.enable(), g && g.enable());
        p && p.onChange();
        h && h.onChange()
      }, onOk:function () {
        ...
        if (b.info) {
          b = b.info;
          if (!this._.selectedElement)for (var g = c.append(l("tbody")), f = parseInt(b.txtRows, 10) || 0, h = parseInt(b.txtCols, 10) || 0, i = 0; i < f; i++)for (var j = g.append(l("tr")), k = 0; k < h; k++) {
            var m = j.append(l("td"));
            CKEDITOR.env.ie || m.append(l("br"))
          }
          f = b.selHeaders;
          if (!c.$.tHead && ("row" == f || "both" == f)) {
            j = new CKEDITOR.dom.element(c.$.createTHead());
            g = c.getElementsByTag("tbody").getItem(0);
            g = g.getElementsByTag("tr").getItem(0);
            for (i = 0; i < g.getChildCount(); i++)h = g.getChild(i), h.type == CKEDITOR.NODE_ELEMENT && !h.data("cke-bookmark") && (h.renameNode("th"), h.setAttribute("scope", "col"));
            j.append(g.remove())
          }
          if (null !== c.$.tHead && !("row" == f || "both" == f)) {
            j = new CKEDITOR.dom.element(c.$.tHead);
            g = c.getElementsByTag("tbody").getItem(0);
            for (k = g.getFirst(); 0 < j.getChildCount();) {
              g = j.getFirst();
              for (i = 0; i < g.getChildCount(); i++)h = g.getChild(i), h.type == CKEDITOR.NODE_ELEMENT && (h.renameNode("td"), h.removeAttribute("scope"));
              g.insertBefore(k)
            }
            j.remove()
          }
          if (!this.hasColumnHeaders && ("col" == f || "both" == f))for (j = 0; j < c.$.rows.length; j++)h =
            new CKEDITOR.dom.element(c.$.rows[j].cells[0]), h.renameNode("th"), h.setAttribute("scope", "row");
          if (this.hasColumnHeaders && !("col" == f || "both" == f))for (i = 0; i < c.$.rows.length; i++)j = new CKEDITOR.dom.element(c.$.rows[i]), "tbody" == j.getParent().getName() && (h = new CKEDITOR.dom.element(j.$.cells[0]), h.renameNode("td"), h.removeAttribute("scope"));
            ...
            {type:"vbox", padding:0, children:[
              {type:"hbox", widths:["5em"], children:[
                {type:"text",
                  ...
                  (a = a.getStyle("width")) && this.setValue(a)
                }, commit:m}
              ]},
              {type:"hbox", widths:["5em"], children:[
          ...
          {type:"vbox", padding:0, children:[
            {type:"text", id:"txtCaption", label:a.lang.table.caption, setup:function (a) {
              ...
              if (0 < a.count()) {
                ...
                e && !e.equals(a.getBogus()) ? (this.disable(), this.setValue(a.getText())) : (a = CKEDITOR.tools.trim(a.getText()),
                  this.setValue(a))
              }
            }, commit:function (d, e) {
              if (this.isEnabled()) {
                var c = this.getValue(), b = e.getElementsByTag("caption");
                if (c)0 < b.count() ? (b = b.getItem(0), b.setHtml("")) : (b = new CKEDITOR.dom.element("caption", a.document), e.getChildCount() ? b.insertBefore(e.getFirst()) : b.appendTo(e)), b.append(new CKEDITOR.dom.text(c, a.document)); else if (0 < b.count())for (c = b.count() - 1; 0 <= c; c--)b.getItem(c).remove()
              }
            }},
