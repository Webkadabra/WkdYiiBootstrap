...
(function () {
  ...
  CKEDITOR.plugins.add("placeholder", {requires:"dialog", lang:"en,bg,cs,cy,da,de,el,eo,et,fa,fi,fr,he,hr,it,nb,nl,no,pl,tr,ug,uk,vi,zh-cn", icons:"createplaceholder,editplaceholder", onLoad:function () {
    CKEDITOR.addCss(".cke_placeholder{background-color: #ffff00;" + (CKEDITOR.env.gecko ? "cursor: default;" : "") + "}")
  }, init:function (a) {
    ...
    a.addMenuItems && (a.addMenuGroup("placeholder", 20), a.addMenuItems({editplaceholder:{label:b.edit, command:"editplaceholder", group:"placeholder", order:1}}), a.contextMenu && a.contextMenu.addListener(function (a) {
      return!a || !a.data("cke-placeholder") ? null : {editplaceholder:CKEDITOR.TRISTATE_OFF}
    }));
    a.on("doubleclick", function (b) {
      if (CKEDITOR.plugins.placeholder.getSelectedPlaceHoder(a))b.data.dialog =
        "editplaceholder"
    });
...
CKEDITOR.plugins.placeholder = {createPlaceholder:function (b, a, c, e) {
  ...
  c && d.setText(c);
  if (e)return d.getOuterHtml();
  a ? CKEDITOR.env.ie ? (d.insertAfter(a), setTimeout(function () {
    a.remove();
    d.focus()
  }, 10)) : d.replace(a) : b.insertElement(d);
  return null
}, getSelectedPlaceHoder:function (b) {
  b = b.getSelection().getRanges()[0];
  b.shrink(CKEDITOR.SHRINK_TEXT);
  for (b = b.startContainer; b && !(b.type == CKEDITOR.NODE_ELEMENT && b.data("cke-placeholder"));)b = b.getParent();
  return b
}};