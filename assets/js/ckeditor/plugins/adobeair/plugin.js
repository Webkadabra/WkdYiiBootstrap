...
(function () {
  function d(a) {
    for (var a = a.getElementsByTag("*"), c = a.count(), b, e = 0; e < c; e++)b = a.getItem(e), function (a) {
        ...
        a.hasAttribute("on" + b) && (a.removeAttribute("on" + b), a.on(b, function (b) {
          ...
          if (f) {
            for (var h = f.length, i, g = 0; g < h; g++) {
              ...
              var d = i.match(/^(["'])([^"']*?)\1$/);
              if (d)f[g] = d[2];
              else if (i.match(/\d+/))f[g] = parseInt(i, 10); else switch (i) {
                case "this":
                  f[g] = a.$;
                  break;
                case "event":
  ...
  CKEDITOR.plugins.add("adobeair", {onLoad:function () {
    CKEDITOR.env.air && (CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(CKEDITOR.dom.document.prototype.write, function (a) {
      ...
      if (a._.panel) {
        ...
        (function () {
          c.isLoaded ? (b = c._.holder, d(b)) : setTimeout(arguments.callee,
            30)
        })()
      } else a instanceof CKEDITOR.dialog && d(a._.element)
    }))
  }, init:function (a) {
