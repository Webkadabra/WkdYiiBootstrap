...
(function () {
  function y(a) {
    for (var a = a.toUpperCase(), b = z.length, d = 0, c = 0; c < b; ++c)for (var e = z[c], f = e[1].length; a.substr(0, f) == e[1]; a = a.substr(f))d += e[0];
    return d
  }

  function A(a) {
    for (var a = a.toUpperCase(), b = B.length, d = 1, c = 1; 0 < a.length; c *= b)d += B.indexOf(a.charAt(a.length - 1)) * c, a = a.substr(0, a.length - 1);
    return d
  }
  ...
  w.removeAnyChildWithName =
    function (a) {
      for (var b = this.children, d = [], c, e = 0; e < b.length; e++)c = b[e], c.name && (c.name == a && (d.push(c), b.splice(e--, 1)), d = d.concat(c.removeAnyChildWithName(a)));
      return d
    };
  ...
  C.firstChild = w.firstChild = function (a) {
    for (var b, d = 0; d < this.children.length; d++)if (b = this.children[d], a(b) || b.name && (b = b.firstChild(a)))return b;
    return null
  };
  w.addStyle = function (a, b, d) {
    var c = "";
    if ("string" == typeof b)c += a + ":" + b + ";"; else {
      if ("object" == typeof a)for (var e in a)a.hasOwnProperty(e) && (c += e + ":" + a[e] + ";"); else c += a;
      d = b
    }
  ...
  CKEDITOR.dtd.parentOf = function (a) {
    var b = {}, d;
    for (d in this)-1 == d.indexOf("$") && this[d][a] && (b[d] = 1);
    return b
  };
  var G = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i, D = /^(?:\b0[^\s]*\s*){1,4}$/, x = {ol:{decimal:/\d+/, "lower-roman":/^m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?x{0,3})(ix|iv|v?i{0,3})$/,
    ...
    if ((d = a.removeAnyChildWithName("cke:listbullet")) && d.length && (d = d[0]))return a.name = "cke:li", b.style && (b.style = E.filters.stylesFilter([
      ["text-indent"],
      ["line-height"],
      ...
      [/^mso-list$/, null, function (a) {
        ...
        1 == a && (d !== v && (b["cke:reset"] = 1), v = d);
        b["cke:indent"] = a
      }]
    ])(b.style, a) || ""), b["cke:indent"] || (p = 0, b["cke:indent"] = 1), CKEDITOR.tools.extend(b, d.attributes), !0;
    v = p = u = null;
    return!1
    ...
    return function (b, d, c) {
      a.setStyle(b, d);
      for (var b = {}, d = c.length, e = 0; e < d; e++)b[c[e]] = a.getStyle(c[e]);
      return b
    }
    ...
    for (var e = a.children, f, h = 0; h < e.length; h++)if (f = e[h], f.name in CKEDITOR.dtd.$listItem) {
      ...
      n.name in
        CKEDITOR.dtd.$list && (a.add(n, h + 1), --i.length || e.splice(h--, 1));
      f.name = "cke:li";
      d.start && !h && (g.value = d.start);
      E.filters.stylesFilter([
        ["tab-stops", null, function (a) {
          (a = a.split(" ")[1].match(G)) && (p = CKEDITOR.tools.convertToPx(a[0]))
        }],
      ...
      a.children = [];
      f = 0;
      for (i = e.length; f < i; f++)a.add(e[f])
    }
    delete a.name;
    d["cke:list"] = 1
  }, assembleList:function (a) {
    for (var b = a.children, d, c, e, f, h, g, a = [], i, n, j, m, k, r, q = 0; q < b.length; q++)if (d = b[q], "cke:li" == d.name)if (d.name = "li", c = d.attributes, j = (j = c["cke:listsymbol"]) && j.match(/^(?:[(]?)([^\s]+?)([.)]?)$/), m = k = r = null, c["cke:ignored"])b.splice(q--, 1); else {
      c["cke:reset"] && (g = f = h = null);
      e = Number(c["cke:indent"]);
      e != f && (n = i = null);
      if (j) {
        if (n && x[n][i].test(j[1]))m =
          n, k = i; else for (var s in x)for (var t in x[s])if (x[s][t].test(j[1]))if ("ol" == s && /alpha|roman/.test(t)) {
          if (i = /roman/.test(t) ? y(j[1]) : A(j[1]), !r || i < r)r = i, m = s, k = t
        } else {
          m = s;
          k = t;
          break
        ...
        !m && (m = j[2] ? "ol" : "ul")
      } else m = c["cke:listtype"] || "ol", k = c["cke:list-style-type"];
      n = m;
      i = k || ("ol" == m ? "decimal" : "disc");
      k && k != ("ol" == m ? "decimal" : "disc") && d.addStyle("list-style-type", k);
      if ("ol" == m && j) {
        switch (k) {
          case "decimal":
            r = Number(j[1]);
      ...
      if (g) {
        if (e > f)a.push(g = new CKEDITOR.htmlParser.element(m)), g.add(d), h.add(g); else {
          if (e < f) {
            f -= e;
            for (var o; f-- && (o = g.parent);)g = o.parent
          }
          g.add(d)
        }
        b.splice(q--, 1)
      } else a.push(g = new CKEDITOR.htmlParser.element(m)), g.add(d), b[q] = g;
      h = d;
      f = e
    } else g && (g = f = h = null);
    for (q = 0; q < a.length; q++)if (g = a[q], s = g.children, i = i = void 0, t = g.children.length, o = i = void 0, b = /list-style-type:(.*?)(?:;|$)/, f = CKEDITOR.plugins.pastefromword.filters.stylesFilter, i = g.attributes, !b.exec(i.style)) {
      for (h = 0; h <
        t; h++)if (i = s[h], i.attributes.value && Number(i.attributes.value) == h + 1 && delete i.attributes.value, i = b.exec(i.attributes.style))if (i[1] == o || !o)o = i[1]; else {
        o = null;
        break
      }
      if (o) {
        for (h = 0; h < t; h++)i = s[h].attributes, i.style && (i.style = f([
          ["list-style-type"]
        ])(i.style) || "");
    ...
    return function (d, c) {
      var e = [];
      (d || "").replace(/&quot;/g, '"').replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function (d, g, f) {
        g = g.toLowerCase();
        "font-family" == g && (f = f.replace(/["']/g, ""));
        for (var n, j, m, k = 0; k < a.length; k++)if (a[k] && (d = a[k][0], n = a[k][1], j = a[k][2], m = a[k][3], g.match(d) && (!n || f.match(n)))) {
          g = m || g;
          b && (j = j || f);
          "function" == typeof j && (j = j(f, c, g));
          j && j.push && (g = j[0], j = j[1]);
    ...
    return function (d) {
      var c = b ? (new CKEDITOR.style(a, b))._.definition : a;
      d.name = c.element;
    ...
    return{elementNames:[
      ...
      var l;
      CKEDITOR.env.gecko && (l = e.applyStyleFilter) && l(a)
    }, $:function (a) {
      var l = a.name || "", e = a.attributes;
      l in d && e.style && (e.style = f([
        [/^(:?width|height)$/, null, q]
      ])(e.style) || "");
      if (l.match(/h\d/)) {
        a.filterChildren();
        if (r(a))return;
        h(c["format_" + l])(a)
      } else if (l in b.$inline)a.filterChildren(), k(a) && delete a.name; else if (-1 != l.indexOf(":") && -1 == l.indexOf("cke")) {
        a.filterChildren();
        if ("v:imagedata" ==
          l) {
      ...
      l in t && (a.filterChildren(), j(a))
    }, style:function (a) {
      if (CKEDITOR.env.gecko) {
        var a = (a = a.onlyChild().value.match(/\/\* Style Definitions \*\/([\s\S]*?)\/\*/)) && a[1], l = {};
      ...
      if (/MsoListParagraph/.exec(a.attributes["class"])) {
        ...
        (b = (b = b && b.parent) && b.attributes) && !b.style && (b.style = "mso-list: Ignore;")
      }
      a.filterChildren();
      r(a) || (c.enterMode == CKEDITOR.ENTER_BR ? (delete a.name, a.add(new CKEDITOR.htmlParser.element("br"))) :
        h(c["format_" + (c.enterMode == CKEDITOR.ENTER_P ? "p" : "div")])(a))
    }, div:function (a) {
      var b = a.onlyChild();
      if (b && "table" == b.name) {
        var c = a.attributes;
      ...
      if (m(a.parent))delete a.name; else {
        a.filterChildren();
        var b = a.attributes, c = b.style, d = a.parent;
        "font" ==
          d.name ? (CKEDITOR.tools.extend(d.attributes, a.attributes), c && d.addStyle(c), delete a.name) : (c = c || "", b.color && ("#000000" != b.color && (c += "color:" + b.color + ";"), delete b.color), b.face && (c += "font-family:" + b.face + ";", delete b.face), b.size && (c += "font-size:" + (3 < b.size ? "large" : 3 > b.size ? "small" : "medium") + ";", delete b.size), a.name = "span", a.addStyle(c))
      }
    }, span:function (a) {
      ...
      if (m(a)) {
        var b = a.firstChild(function (a) {
          ...
          d = (b = b && (b.value || "l.")) && b.match(/^(?:[(]?)([^\s]+?)([.)]?)$/);
        if (d)return b = i(d, b), (a = a.getAncestor("span")) && / mso-hide:\s*all|display:\s*none /.test(a.attributes.style) && (b.attributes["cke:ignored"] = 1), b
      }
      if (d = (b = a.attributes) && b.style)b.style = f([
        ["line-height"],
      ...
      var b = a.attributes;
      b && !b.href && b.name ? delete a.name : CKEDITOR.env.webkit && (b.href && b.href.match(/file:\/\/\/[\S]+#/i)) && (b.href = b.href.replace(/file:\/\/\/[^#]+/i, ""))
      ...
      [/^margin$|^margin-(?!bottom|top)/, null, function (a, b, d) {
        if (b.name in{p:1, div:1}) {
          ...
          if ("margin" == d)a = s(d, a, [b])[b]; else if (d != b)return null;
      ...
      function (a, b) {
        ...
        return d ? (d = (c = d[1] || c && "l.") && c.match(/>(?:[(]?)([^\s]+?)([.)]?)</), i(d, c)) : CKEDITOR.env.gecko && c ? (c = CKEDITOR.htmlParser.fragment.fromHtml(c[0]).children[0], (d = (d = (d = b.previous) && d.value.match(/<v:imagedata[^>]*o:href=['"](.*?)['"]/)) && d[1]) && (c.attributes.src = d), c) : !1
      } : a}
  }}, F = function () {
