...
(function () {
  ...
  function h(a) {
    delete g[a.name];
    CKEDITOR.tools.isEmpty(g) && (clearTimeout(i), i = 0);
    a = a.editable();
    a._.shadow && (a.setOpacity(1), a._.shadow.remove(), delete a._.shadow)
  }
    ...
    i || function () {
      ...
      i = setTimeout(function () {
        for (var e in g) {
          ...
          f = b.editable();
          for (var k = "", l = 0, m = ["width", "height"]; l < m.length; l++)k += "|" + f.getSize(m[l]);
          f = {html:f.getHtml(), size:k};
          d && (f.html != d.html ? (h(b), j(b)) : f.size != d.size && (b = b.editable(), d = b._.shadow, d.setStyles({"z-index":-1, position:"absolute", left:b.$.offsetLeft + "px", top:b.$.offsetTop + "px", width:b.getSize("width", 1) + "px", height:b.getSize("height", 1) + "px"}), d.$.scrollTop = b.$.scrollTop));
          g[e] = f
        }
        a()
      }, 200)
  ...
  if (CKEDITOR.env.ie && 8 > CKEDITOR.env.version)CKEDITOR.plugins.add("selectionmark",
    function () {
    }); else {
    var g = {}, i;
    CKEDITOR.plugins.add("selectionmark", {init:function (a) {
      a.on("instanceReady", function () {
        var c = a.editable();
        if (!("absolute" == c.getComputedStyle("position") || /^inline/.exec(c.getComputedStyle("display"))))for (var e = ["top", "bottom"], b = 0, d; d = e[b], 2 > b; b++) {
          var f = parseInt(c.getComputedStyle("border-" + d + "-width"), 10), g = parseInt(c.getComputedStyle("padding-" + d), 10);
          !f && !g && (d = "border-" + d, c.setStyle(d, "1px solid transparent"))
        }
          ...
          setTimeout(function () {
            if (a.focusManager.hasFocus) {
              ...
              if (b && b.isLocked) {
                ...
                if (c = b)b = b.getRanges(), c = !(1 == b.length && b[0].collapsed);
                c && (j(a), n(a))
              }
            }
          }, 200)
