...
class Bootstrap extends CApplicationComponent
{
	...
	public function registerResponsiveCss()
	{
		...
		$this->assetsRegistry->registerCssFile($this->getAssetsUrl() . '/css/bootstrap-responsive' . (!YII_DEBUG ? '.min' : '') . '.css');
	}
	...
	public function registerInputmask($selector = null, $options = array())
	{
		$this->registerPlugin(self::PLUGIN_MASK, $selector, $options);
		$this->registerAssetJs('bootstrap-inputmask.min.js');
	}
	...
	private function makeBootstrapCssFilename()
	{
		...
		if ($this->enableCdn && $this->responsiveCss && $this->minifyCss) { // CDN hosts only responsive minified versions
